...
namespace SCMM.Web.Server.Extensions
{
    public static class ClaimsPrincipalExtensions
    {
        ...
        {
            Guid id;
            Guid.TryParse(user?.FindFirst(SCMM.Web.Data.Models.ClaimTypes.Id)?.Value, out id);
            return id;
        }

        public static string SteamId(this ClaimsPrincipal user)
        {
            return user?.FindFirst(SCMM.Web.Data.Models.ClaimTypes.SteamId)?.Value;
        }

        public static string Name(this ClaimsPrincipal user)
        {
            return user?.FindFirst(SCMM.Web.Data.Models.ClaimTypes.Name)?.Value ?? user.Identity?.Name;
        }

        public static string Language(this ClaimsPrincipal user)
        {
            return user?.FindFirst(SCMM.Web.Data.Models.ClaimTypes.Language)?.Value;
        }

        public static string Currency(this ClaimsPrincipal user)
        {
            return user?.FindFirst(SCMM.Web.Data.Models.ClaimTypes.Currency)?.Value;
        }
    }
}
